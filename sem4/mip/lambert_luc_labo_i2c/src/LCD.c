...
void disp_img(img_t* img){
    set_window(0, 0, img->width-1, img->height-1);
    uint16_t dat;
    int cpt=0;
    for (int i=0; i<img->height; i++) {
        for (int j=0; j<img->width; j++) {
            pixel_t px = img->pix2d[i][j];
            dat=RGB(px.r,px.g,px.b)>>8 | RGB(px.r,px.g,px.b);
            Write_Cmd_Data16(dat);
        }
    }
    free_img(img);
}
