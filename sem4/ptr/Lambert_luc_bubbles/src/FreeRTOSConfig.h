...
#define configUSE_IDLE_HOOK			0	//configHEPIA_TRACING tracing by interrupts (no time for a background task!)
#define configUSE_TICKLESS_IDLE		0


#if configHEPIA_TRACING
// TRACE WITHOUT TRACING TASK:
// condition task_id!=uxTaskNumber can be useful to avoid tracing the tracing task itself!
#define traceTASK_SWITCHED_IN() {if (previous_id_in!=pxCurrentTCB->task_id) {\
		if (previous_id_in!=0xFF && previous_id_in!=uxTaskNumber) \
			write_trace_ref(previous_id_in, 0); \
		if (pxCurrentTCB->task_id!=uxTaskNumber) \
